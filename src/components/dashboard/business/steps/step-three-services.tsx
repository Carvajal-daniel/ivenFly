...
const isFoodClothingNiche = (n: string): n is (typeof FOOD_CLOTHING_NICHES)[number] =>
  (FOOD_CLOTHING_NICHES as readonly string[]).includes(n);

export function StepThreeServices({ form, selectedNiche }: StepThreeServicesProps) {
  // Qualquer uma das linhas abaixo resolve:
  // const showDelivery = selectedNiche && FOOD_CLOTHING_NICHES_LIST.includes(selectedNiche);
  const showDelivery = !!selectedNiche && isFoodClothingNiche(selectedNiche);
  ...
  const isOtherAmenitiesFilled = !!watchedValues.amenities?.otherAmenities?.length;
  const isAmenitiesFilled = hasAnyAmenity || isOtherAmenitiesFilled;

  const isEmployeesFilled =
    watchedValues.employees !== undefined && watchedValues.employees !== null && watchedValues.employees > 0;

  const isDeliveryFilled = watchedValues.delivery !== undefined;

  const employeesError = !!form.formState.errors.employees;

  return (
    <div className="max-w-[50rem] mx-auto space-y-6">
      {/* Serviços e Produtos */}
      <div className={`p-4 rounded-xl transition-all duration-300 ${isServicesFilled ? "bg-primary/5" : ""}`}>
      ...
      {/* Comodidades */}
      <div className={`p-4 rounded-xl transition-all duration-300 ${isAmenitiesFilled ? "bg-primary/5" : ""}`}>
                    ...
                    <FormItem>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <FormLabel className="font-semibold text-sm cursor-pointer">{amenity.label}</FormLabel>
                        </div>
                        ...
                        <FormControl>
                          <Checkbox
                            id={amenity.name}
                            checked={!!field.value}
                            // shadcn Checkbox envia boolean | "indeterminate"; normalizamos para boolean
      ...
      <div className={`p-4 rounded-xl transition-all duration-300 ${isEmployeesFilled ? "bg-primary/5" : ""}`}>
        <FormField
          control={form.control}
          name="employees"
          render={({ field }) => (
            <FormItem className="space-y-3">
              <FormLabel className="text-lg font-semibold flex items-center gap-2">
                ...
                Número de Funcionários <span className="text-red-500" aria-hidden="true">*</span>
                <InfoTooltip description="Total de colaboradores, incluindo o proprietário." />
                {isEmployeesFilled && !employeesError && (
                  <div className="ml-auto">
                    <Check className="h-5 w-5 text-green-600" />
                  </div>
                )}
              ...
              <FormControl>
                <Input
                  ...
                  required
                  aria-required="true"
                  aria-invalid={employeesError || undefined}
                  className="h-12 text-base border-border/40 focus:border-primary transition-colors"
                />
              </FormControl>
              <FormMessage />
      ...
      {showDelivery && (
        <div
          className={`p-4 rounded-xl transition-all duration-300 ${
            isDeliveryFilled && watchedValues.delivery ? "bg-primary/5" : ""
          }`}
        >
          <FormField
            control={form.control}
            ...
            render={({ field }) => (
              <FormItem>
                <label htmlFor="delivery-checkbox" className="flex items-center gap-3 cursor-pointer">
                  <FormControl>
                    <Checkbox
                  ...
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <FormLabel className="text-lg font-semibold cursor-pointer">Oferece Entrega/Delivery</FormLabel>
                    <InfoTooltip description="Marque se possui serviços próprios ou utiliza aplicativos de entrega." />
                  </div>
                  {field.value && <Check className="h-5 w-5 text-green-600" />}
                </label>
                <FormMessage />
              </FormItem>
            )}
